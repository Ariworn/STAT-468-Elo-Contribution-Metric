...
The motivating idea is to use a Bayesian Hierarchical Model (partial pooling) that nests parameters inside parameters, and as such they are viewed as a sample from a probability distribution of parameters (taking a lot of this from pymc's multilevel modelling module). In a completely pooled model we assume that player skill is drawn from the same model and thus it ignores the variance between the sampling units. In a model with no pooling however, we assume the player skill are drawn from separate models, that the players are not similar in any way. A partial pooling or Hierarchical model combines both into one.

Below I fit two models with pymc that attempts to approximate the posterior distribution using an MCMC sampler method. Initially I set up 4 models, each with slightly different hyperparameters in an effort to find one that makes sense. The ones that HalfNormal distribution helped the model in converging. 

I did some prior predictive checks to see whether the simulated delta_elo values were reasonable, and only the ones with HalfNormal were giving values on the tails that were reasonable. The other HalfCauchy distribution would give values of +1000 delta elo even for a common percentile.
